...
"""
    ...
    def create_text_for_post_code_best_combination(
        ...
        return (
            f"**🔍 У нас есть средство**: \"{data['Средства']['Исходное_средство'][0]}\".\n"
    ...
    def create_text_for_post_code_analog(
        ...
        return (
            f"🔍 **У нас есть средство - \"{data['Средства']['Исходное_средство'][0]}\".\n"
            f"Является ли \"{data['Средства']['Аналог_средство'][0]}\" его аналогом "
...
{data['Средства и артикулы']}

{data['Хештег']}

⚠️ Вся информация предоставляется исключительно в образовательных целях и не является медицинским или юридическим заключением. Мы основываемся на анализе составов косметических средств с помощью искусственного интеллекта, обученного на открытых научных источниках, данные которых могут обновляться. Перед использованием продукта рекомендуем проконсультироваться с врачом или специалистом."""
