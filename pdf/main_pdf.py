...
class PDFCreator:
    ...
    def split_product_info_to_pages(
            self,
            product_info: dict,

    ) -> list[dict]:
        """
        Делает из одного продукта список словарей по страницам для генерации 6 страниц PDF.
        """

        pages = []

        # СТРАНИЦА 1: фиксированные данные
        pages.append(
            {
                "Изображение": product_info.get("Ссылка на изображение в базе", ""),
                "Заголовок": "Разбор состава",
                "Тип продукта": product_info.get("Тип продукта", ""),
                "Название": product_info.get("Название", ""),
                "Соотношение": product_info.get("Соотношение", ""),
            }
        )

        content_fields = {
            key: value for key, value in product_info.items()
            if key not in ["Ссылка на изображение в базе", "Название", "Соотношение", "title"]
        }


        # 5 страниц для оставшихся данных
        # Делаем равномерное распределение
        fields_per_page = max(1, len(content_fields) // 5)

        temp_page = {}

        for index, (key, value) in enumerate(content_fields.items(), 1):
            temp_page[key] = value

            # Закрываем страницу
            if index % fields_per_page == 0 or index == len(content_fields):
                pages.append(temp_page)
                temp_page = {}

        return pages

    def _fill_flowables_one_product(
            self,
            page_data: dict,
    ) -> None:
        """
        Метод для создания flowables на основе данных одной страницы
        (по сути, добавляет контент для одной страницы продукта).

        :param page_data: словарь с данными для конкретной страницы
        """

        # Добавляем изображение (если есть)
        if page_data.get("Изображение"):
            self._add_product_image(page_data["Изображение"])

        # Добавляем заголовок (если есть)
        if page_data.get("Заголовок"):
            self.flowables.append(Paragraph(page_data["Заголовок"], self.my_style['title_style']))

        # Добавляем тип продукта (если есть)
        if page_data.get("Тип продукта"):
            self.flowables.append(
                Paragraph(f"<b>Тип продукта:</b> {page_data['Тип продукта']}",
                          self.my_style['normal_style']))

        # Добавляем название и соотношение
        if page_data.get("Название"):
            self.flowables.append(Paragraph(page_data["Название"], self.my_style['title_style']))

        if page_data.get("Соотношение"):
            self.flowables.append(Paragraph(f"{page_data['Соотношение']}",
                                            self.my_style['ratio_align_style']))

        # Пробегаемся по остальным полям и добавляем их как блоки текста
        skip_keys = ["Изображение", "Заголовок", "Тип продукта", "Название", "Соотношение"]

        for key, value in page_data.items():
            if key not in skip_keys:
                self.flowables.append(Spacer(1, 15))
                self.flowables.append(Paragraph(f"<b>{key}:</b>", self.my_style['bold_style']))
                self.flowables.append(Paragraph(value, self.my_style['normal_style']))

        # Добавляем разрыв страницы, если дальше будут flowables
        self.flowables.append(Spacer(1, 40))
    ...
    def gen_pages_for_one_product(
        ...
        :return: None
        """
        print('▶️ Старт генерации PDF и JPG для одного продукта')

        self.sizes = (1024, 1280)

        # Разбиваем инфу на 6 страниц
        product_pages_info = self.split_product_info_to_pages(info)
        ...
        # Создаем шаблон страницы с брендингом
        template = self._create_page_template(
            frames=[frame, ],
            on_page_end=self._on_page_end_brand_line_wrapper(product=info),
        )

        # Добавляем шаблон к документу
        doc.addPageTemplates([template])

        # Очищаем flowables и добавляем все страницы
        self.flowables = []
        for page_data in product_pages_info:
            self._fill_flowables_one_product(page_data)
            self.flowables.append(Spacer(1, 50))  # Добавим отступ между страницами
