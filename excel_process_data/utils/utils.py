...
def creating_dict_for_pdf(
    ...
    Название средства : {
                        ...
                        Лучшее средство: описание,
    }
    }
    :param dict_full_info: полный словарь со всей информацией по средствам
    :return: словарь с информацией по средствам с выборочной информацией для вставки в картинку
    """

    # пределение итогового словаря
    dict_for_picture = {}

    # ключи, которые нужно взять из полного словаря
    selective_keys = [
        "Плюсы",
        "Минусы",
    ...
    # итерируемся по полному словарю
    for product_name, attributes in dict_full_info.items():
        if product_name == "итоговая рекомендация":
            continue

        # создаем ключ с названием продукта и значением равным пустому словарю
        dict_for_picture[product_name] = {}
        # итерируемся по содержимому исходного словаря с нужными ключами
        for key, value in attributes.items():
            if key in selective_keys:
                dict_for_picture[product_name][key] = value
